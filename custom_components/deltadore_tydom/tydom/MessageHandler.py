                    ...
                    try:
                        ...
                        for elem in endpoint["cdata"]:
                            if type_of_id == "conso":
                                ...
                                # period=YEAR&periodOffset=0&dest=<dest>, so its
                                # response also carries a "dest" parameter and
                                # would otherwise be misrouted to the plain
                                # counter branch below.
                                if "parameters" in elem and elem["parameters"].get(
                                    ...
                                    # energyInstant?unit=ELEC_A / ELEC_W: instant
                                    # power/current reading, not a counter.
                                    # Response shape: {"date": ..., "measure": <val>,
                                    # "min": ..., "max": ..., "minScale": ...,
                                    # "maxScale": ...}.
                                    values = elem.get("values") or {}
                                    if "measure" in values:
                                        element_value = values["measure"]
                                    elif "value" in values:
                                        element_value = values["value"]
                                        ...
                                        continue
                                    # ELEC_A is reported in hundredths of an amp
                                    # (confirmed against a Linky VA reading).
                                    if elem["parameters"]["unit"] == "ELEC_A":
                                        element_value = element_value / 100
                                    element_name = (
                                        elem["name"] + "_" + elem["parameters"]["unit"]
                                    )
                                    data[element_name] = element_value
                                else:
                                    continue
